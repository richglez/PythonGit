...
def TresPrimerosLugares():
    mayor = 0
    archivo = open("atletas.txt",'r')
    for atletas_valores in archivo.readlines():
        atletas_valores=atletas_valores.split (sep=',')
        if mayor<float(atletas_valores[3]):
            mayor=float(atletas_valores[3])
        print("Nombre:",atletas_valores[1],"\ncalificacion",float(atletas_valores[3]))
    print("La calificacion mayor es",mayor)
    archivo.close()
